...
module.exports = createCoreController("api::order.order", ({ strapi }) => ({
    ...
    async find(ctx) {
        ...
        ctx.query = { ...ctx.query, filters: { user: user?.id } };
    ...
    //     let entities;
    //     if (ctx.query._q) {
    //         entities = await strapi.service("api::order.order").search({...ctx.query, user: user?.id});
    //     } else {
    //         entities = await strapi.service("api::order.order").find({...ctx.query, user: user?.id});
    //     }
    ...
    // Initialize the Strapi SDK with your API URL and authentication token
    // const apiUrl = 'http://localhost:1337';
    // const token = "";
    // const strapiClient = strapi(apiUrl, { headers: { Authorization: `Bearer ${token}` } });

    // // Get the logged in user ID
    // const { id: userId } = await strapiClient.get('/users/me');

    // // Find the orders for the logged in user
    // const orders = await strapiClient
    //     .query('orders')
    //     .find({ user: userId });
